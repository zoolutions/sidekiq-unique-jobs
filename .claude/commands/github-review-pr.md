...
3. Resolve every conflicted file **semantically** — read both sides and produce the version that preserves BOTH changes' intent. Never blanket `--ours`/`--theirs` a source file. Repo-specific rules:
   ...
   - **Tracked lockfiles — any committed `Gemfile.lock` / `bun.lock`** (today `myapp/`; the gem root's `Gemfile.lock` and `gemfiles/*.lock` are gitignored and can never conflict): take the base's file, then re-resolve in that lockfile's directory — `bundle install` for a Gemfile.lock, `bun install` for a bun.lock — so the branch's own dependency changes, if any, re-resolve on top. Never hand-edit a lockfile.
